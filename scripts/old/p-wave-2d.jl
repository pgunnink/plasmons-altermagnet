...
ϵ_natural(k, σ, Js, Δ) = (k[1]^2 + k[2]^2) * Js + σ * Δ * k[1]

##
function f_iter(k, ω, q, σ, Js, Δ, Ef, kmax2)
    ϵk = ϵ_natural(k, σ, Js, Δ)
    if ϵk > Ef || (k[1]^2 + k[2]^2) > kmax2
        return 0im
    else
...
function χ0_natural(ω, q, σ, Js, Δ, Ef)
    klim = 1
    (2pi)^(-3) .* hcubature(x -> f_iter(x, ω, q, σ, Js, Δ, Ef, klim^2), [-klim, -klim], [klim, klim], initdiv=5, norm=x -> sqrt(real(x)^2), rtol=1e-3, maxevals=Int(1e8))
end
##
...
function epsilon_natural(ω, q, Js, Δ, vq, Ef)
    ...
    (2pi)^(-3) .* hcubature(x -> 1.0 - vq / norm(q)^2 * (f_iter(x, ω, q, 1, Js, Δ, Ef, klim^2) + f_iter(x, ω, q, -1, Js, Δ, Ef, klim^2)), [-klim, -klim], [klim, klim], rtol=1e-3, maxevals=Int(1e8), norm=x -> sqrt(real(x)^2))
end
...
θ = pi / 2
ϕ = 0pi / 2
qloc = SVector(q * sin(θ) * cos(ϕ), q * sin(θ) * sin(ϕ), q * cos(θ))[1:2]
